...
class COVIDImagesSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # samples_per_label = int(self.batch_size / 2)
        # covid1_indices = np.random.choice(np.argwhere(self.y == 1)[:, 0], size=samples_per_label, replace=False)
        # covid0_indices = np.random.choice(np.argwhere(self.y == 0)[:, 0], size=samples_per_label, replace=False)
